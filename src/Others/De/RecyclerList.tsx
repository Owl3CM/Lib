...
export default class RecyclerList extends React.Component<IRecyclerScroller> {
    ...
    initGrid = (recycler:RecyclerList) => {
        recycler.grid.replaceChildren(...recycler.service.items.slice(0, this.initItemsToCalculate).map((item:any) => this.buildItem({ item })));
        recycler.handleCalculations();
        recycler.grid.append(...recycler.service.items.slice(this.initItemsToCalculate, this.viewedItems).map((item:any) => this.buildItem({ item })));
        recycler.handleCalculations();
    };
